...
export default function ResultsPage() {
  ...
  }, [cashFlowData, selectedAssets])

  const fetchResultsData = async () => {
    try {
      setLoading(true)
      
      const [cashFlowResponse, assetsResponse] = await Promise.all([
        fetch('/api/results/cash-flows'),
        fetch('/api/results/assets')
      ])

      if (!cashFlowResponse.ok || !assetsResponse.ok) {
  ...
  const calculateSummaryStats = () => {
    ...
    const stats = {
      totalRevenue: filteredData.reduce((sum, item) => sum + (item.revenue || 0), 0),
      totalOpex: filteredData.reduce((sum, item) => sum + (item.opex || 0), 0),
      totalCapex: filteredData.reduce((sum, item) => sum + (item.capex || 0), 0),
      totalEquityCashFlow: filteredData.reduce((sum, item) => sum + (item.equity_cash_flow || 0), 0),
      avgMonthlyRevenue: 0,
      periods: filteredData.length
    }

    // Group by date to get unique periods
    const groupedByDate = filteredData.reduce((acc, item) => {
      const date = item.date
      if (!acc[date]) {
        acc[date] = { revenue: 0, opex: 0, equity_cash_flow: 0 }
      }
      acc[date].revenue += item.revenue || 0
      acc[date].opex += item.opex || 0
      acc[date].equity_cash_flow += item.equity_cash_flow || 0
      return acc
    }, {})

    const uniquePeriods = Object.keys(groupedByDate).length
    stats.avgMonthlyRevenue = uniquePeriods > 0 ? stats.totalRevenue / uniquePeriods : 0
  ...
  const getFilteredData = () => {
    ...
    if (selectedAssets.length > 0) {
      filtered = filtered.filter(item => selectedAssets.includes(item.asset_id))
    }
  ...
  return (
    <div className="p-6">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-600 mt-2">Analyze monthly cash flow performance by asset</p>
      </div>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-xl font-bold text-gray-900">{formatCurrency(summaryStats.avgMonthlyRevenue)}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
        ...
        {chartData.length > 0 ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-gray-600">Select assets to view cash flow analysis</p>
          </div>
        )}
      </div>
            ...
            <table className="w-full">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">CFADS</th>
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-gray-200">
                {selectedAssets.map(assetId => {
                  const asset = assetList.find(a => a.asset_id === assetId)
                  const assetData = getFilteredData().filter(d => d.asset_id === assetId)
                  ...
                  const totals = assetData.reduce((acc, item) => ({
                    ...
                    cfads: acc.cfads + (item.cfads || 0)
                  }), { revenue: 0, opex: 0, equity_cash_flow: 0, cfads: 0 })
