...
export async function GET() {
  try {
    ...
    portfolios.forEach(p => {
      ...
      // Add this portfolio's info - use PortfolioTitle as primary, PlatformName as fallback
      const portfolioInfo = {
        name: p.PortfolioTitle || p.PlatformName || p.unique_id,
        title: p.PortfolioTitle || p.PlatformName || p.unique_id
      };
    ...
    const portfolioDetails = Object.values(portfoliosByUniqueId)
      .map(group => ({
        ...
        // Use PortfolioTitle as the only display name
        name: group.portfolios[0]?.name || group.unique_id,
        title: group.portfolios[0]?.title || group.unique_id,
